<script lang="ts" setup>
  ...
  onMounted(() => {
    ...
    const wrapper = document.querySelector("#aboutWrapper") as HTMLElement;
    inView(
      wrapper,
      (entry) => {
...
<template>
  <div id="" class="relative">
    <div
      id="aboutWrapper"
      class="flex min-h-screen flex-col items-center justify-center gap-24 overflow-visible py-20 opacity-0"
    >
      ...
      <div id="scrollContainer" class="grid h-full w-full translate-x-24 grid-cols-1 lg:grid-cols-3 lg:gap-20">
        ...
        >
          <div v-for="(topic, jdex) in col" :key="jdex" class="row relative pb-10" :class="`row-${jdex}`">
