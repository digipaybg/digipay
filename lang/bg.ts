export default {
  home: "Начало",
  aboutus: "За нас",
  blog: "Блог",
  contact: "Контакти",
  day: "Ден",
  conference: "Конференция",

  themes: "Теми",
  speakers: "Лектори",
  system: "Системно",
  dark: "Тъмна",
  light: "Светла",
  partners: "Партньори",
  heroTitle: "Най-голямата конференция за финтех в България",
  upcoming: "НОВО СЪБИТИЕ 2024",
  buyTicket: "Купи билет",
  learnMore: "Научи повече",
  heroDescription:
    "Конференцията DIGIPAY 2024 ще зададе тенденцията и ще ни подразни с нови технологии. Следвайте и регистрирайте се, за да изследвате повече модерни иновации",
  location: "Интер Експо Център, София",
  agenda: "Програма",
  contacts: "Контакти",
  october: "Октомври",
  liveOrOnline: "На живо или онлайн",
  instantPayments: "Незабавни плащания",
  paymentScam: "Предотвратяване на измами при плащания",
  cloudInnovationsForFinance: "Облачни технологии във финансовата индустрия",
  antiMoneyLaunderingMeasures: "Регулации и практики срещу пране на пари",
  userPaymentAI: "Потребителско преживяване и AI при разплащанията",
  foreignQuotaEntry: "Промени при плащанията при влизането в еврозоната",
  secureDigitalPayments: "За по-сигурни и удобни дигитални плащания",
  newBusinessModelsInPayments: "Нови бизнес модели в разплащанията",
  identityManagement: "Управление на самоличността и достъпа",
  electronicSignatureSolutions: "Решения за електронен подпис",
  cloudTechnologies: "Облачни технологии",
  paymentMobileApps: "Мобилни приложения за разплащания и банкови услуги",
  openBankingAndRealTimePayments: "Отворено банкиране и плащания в реално време",
  europeanRegulations: "Европейски и национални регулации",
  paymentInfrastructureModernization: "Модернизация на платежната инфраструктура",
  machineLearning: "Машинно обучение и изкуствен интелект",
  newTechnologyTrends: "Иновации, Стартъп екосистема, Устойчиви инвестиции",
  fraudDetection: "Решения за откриване и предотравяне на измами и сигурност при разплащанията",
  antiMoneyLaunderingSolutions: "Решения срещу прането на пари",
  dataSecurity: "Сигурност на данни: Цифрова идентичност; Цифрово включване",
  blockchain: "Блокчейн; Криптовалута; Нови дигитални пари",
  specializedEvent: "DIGIPAY е специализираното събитие за разплащания в банковия, финансовия и инвестиционния сектор.",
  platformDiscussion:
    "DIGIPAY е платформата за обсъждане на решенията за цифрово банкиране и плащания, откриване и предотвратяване на измами при плащания в реално време.",
  digitalPaymentsPlatform: "DIGIPAY е платформата за промяна на света на парите и дигиталните плащания.",
  enhancedAwareness:
    "DIGIPAY прави бизнес за бизнеса, регулаторите за повишаване на информираността в отговор на нарастващите очаквания на потребителите.",
  paymentPlatform:
    "DIGIPAY търси силно партньорство за сигурни и удобни дигитални плащания в България и страните от Централна и Източна Европа.",
  showBetterClientSolutions:
    "Да покажем заедно по-добри решения за клиенти и крайни потребители в сферата на плащанията.",
  convertToAdvantage: "Да превърнете вашата кауза в конкурентно предимство.",
  customerTrainingMission:
    "Да инвестирате в стратегическата мисия за обучение на клиентите в по-сигурни практики в цифровите плащания.",
  eventSpecializedSecurePayments: "Ние сме специализирано събитие с кауза за по-удобни и сигурни плащания",
  bankingAndFinTechSectorNameGenerator:
    "Да утвърдим заедно името на банковия и финтех сектор като генератори на устойчиви финанси в България.",
  bio: "Био",
  eventAbout: "За какво е събитието?",
  ourPartners: "Нашите партньори",
  general: "Генерални",
  organizational: "Организационни",
  content: "Съдържание",
  lastYear: "Как премина DIGIPAY 2023?",
  attendees: "Участници",
  investors: "Инвеститори",
  aceriBulgaria: "Асоциация на сертифицираните експерти по разкриване на измами",
  basscom: "Българска асоциация на софтуерните компании",
  bgFintech: "Българската Финтек Асоциация",
  acfe: "ACFE България",
  bea: "Българската E-Commerce Aсоциация",
  baseh: "Българската асоциация на сертифицираните етични хакери",
  bsa: "Българската Стартъп Асоциация",
  safenet: "Национален център за по-безопасен интернет - safenet",
  pastYears: "Какво се случи в миналите години?",
  allVideos: "Всички видеа",
  video: "Видео от DIGIPAY 2023",
  readingTime: ({ named }: { named: (key: string) => number }) =>
    named("minutes") === 1 ? "1 минута четене" : `${named("minutes")} минути четене`,
  blogTitle: "DIGIPAY Блог",
  morePosts: "Още публикации",
  toc: "Съдържание",
  advisoryboard: "Консултативен съвет",
  stoilka: "Стоилка Арсова",
  stoilkaPosition: "Директор на Националната карта и платежна схема, част от БОРИКА АД",
  svetlin: "Светлин Лазаров",
  svetlinPosition: "Ръководител на отдел 'Кибер разузнаване и цифрова форензика' в ГДОКО",
  goran: "Горан Ангелов",
  goranPosition: "Основател и изпълнителен директор, IBS България",
  boris: "Борис Гончаров",
  borisPosition: "Главен визионер, AMATAS",
  miglen: "Миглен Евлогиев",
  miglenPosition: "Основател, Българска фондация за киберсигурност Вицепрезидент, Киберсигурност, Payhawk",
  pavel: "Павел Камински",
  pavelPosition: "Основател, 7Security",
  readAllPosts: "Прочети всички публикации",
  backPosts: "Всички публикации",
  different: "Защо DIGIPAY се отличава?",
  invest: "Защо да инвестирате в DIGIPAY?",
  themes2024: "Топ темите 2024",
  previousEditions: "Предишни издания",
  raya_lecheva: "Рая Лечева",
  petya_veleva: "Петя Велева",
  ceo: "Изпълнителен директор",
  founderAndGeneralManager: "Основател и генерален мениджър",
  contactUs: "Свържете се с нас",
  contactus: "Свържете се с нас",
  name: "Име",
  email: "Имейл",
  message: "Съобщение",
  send: "Изпрати съобщението",
  subscribe: "Абонирай се",
  subscribeText: "Абонирай се за нашата бюлетина",
  yourName: "Твоето Име",
  yourEmail: "Твоят Имейл",
  yourMessage: "Твоето Съобщение",
  mailSent: "Вашето съобщение беше изпратено успешно",
  mailError: "Възникна грешка при изпращането на съобщението",
  topic: "Тема",
  media: "Медийни",

  // Agenda translations
  discussion: "ДИСКУСИЯ",
  registration: "Регистрация и кафе",
  openingKeynotes: "ОТКРИВАЩИ ПРЕЗЕНТАЦИИ",
  instantPaymentsAndInnovation: "НЕЗАБАВНИ ПЛАЩАНИЯ И ПЕРСПЕКТИВИ ЗА ИНОВАЦИИ",
  coffeeBreak: "Кафе пауза",
  poweredBy: "Осигурено от",
  keynoteSession: "КЛЮЧОВИ ЛЕКЦИИ",
  panelDiscussion: "ПАНЕЛНА ДИСКУСИЯ",
  practicalAspectsFinancialFraud: "Практически аспекти на предотвратяването и справянето с финансови измами",
  keynoteSpeech: "ПРЕЗЕНТАЦИЯ",
  lunchTime: "ОБЕДНА ПОЧИВКА",
  discussionCroatiaEuro:
    "Успешните стратегии на Хърватия в навигирането на банковия и финансовия сектор при преминаването към еврото",
  solutionsAgainstFinancialCrime: "Решения срещу финансови престъпления и пране на пари на фокус",
  firechatSession: "FIRECHAT",
  convenientOrSecurePayments: "Удобни или сигурни дигитални плащания",
  closingCocktail: "ЗАКРИВАЩ КОКТЕЙЛ",

  // Positions and organizations
  dgFinancialStability:
    "Генерална Дирекция „Финансова стабилност, финансови услуги и съюз на капиталовите пазари”, Европейска комисия",
  governorBNB: "Управител, Българска народна банка, Поканен",
  fintechApplications: "Финтех приложения и решения, Сирма Груп Холдинг",
  directorNationalCard: "Директор Национална карта и платежна схема, БОРИКА АД",
  seniorManagerDigitalBanking: "Старши мениджър Дигитално банкиране, УниКредит Булбанк",
  countryInnovationLeader: "Лидер по иновации за страната, КВС България",
  headCyberIntelligence:
    "Ръководител на отдел 'Кибер разузнаване и цифрова форензика' в Главна дирекция за борба с организираната престъпност (ГДБОП)",
  seniorDirectorCardBusiness: `Главен Директор на дирекция „Картов бизнес" в БОРИКА+Гост Общинска банка`,
  founderCEO: "Основател и Главен изпълнителен директор, IBS България",
  founderCyberSecurityTalks: "Основател на Cyber Security Talks Bulgaria",
  chiefVisionaryOfficer: "Главен визионер, AMATAS",
  chiefTechnologyOfficer: "Главен технологичен директор и лидер по технически продажби, IBM Централна и Източна Европа",
  headAIPractice: "Ръководител на практиката по изкуствен интелект, Infopulse България",
  coFounder: "Съосновател",
  businessIntelligenceConsultant: "Консултант по бизнес интелигентност, KOIOS Хърватия",
  phdFounderDirector:
    "Доктор, Европейски университетски институт, Основател и директор на CEE Център за дигитални финанси",
  vicePresidentACFE: "Вицепрезидент, Асоциация на сертифицираните експерти по разкриване на измами (ACFE), Кипър",
  directorHeadRegionalDigital: "Директор, Ръководител на регионални дигитални и консултантски услуги по риска",
  seniorVicePresident: "Старши вицепрезидент",
  founderCEOFram: "Основател и Главен изпълнителен директор, FRAM CREATIVE SOLUTIONS",
  seniorVicePresidentSirma: "Старши вицепрезидент, Sirma Financial Industry",
  founderCEO7Security: "Основател и Главен изпълнителен директор, 7Security",
  fintechAssociationChairman: "Председател на Българска Финтех Асоциация",

  // Specific topics
  topicImmediatePayments:
    "Незабавни плащания, сигурност на платежните услуги, иновации и след влизането в сила на PSD3, бъдещи предизвикателства пред отворените финанси.",
  topicFinancialFraud: "Седем години назад и напред във финансовите измами",
  topicFinancialCrimePrevention:
    "Превенция на финансови престъпления като услуга - Мониторинг на измами и проверка на санкции",
  topicHolisticSolution:
    "Кои са елементите, които ни дават цялостно решение за защита и какви са основните предизвикателства и как да ги посрещнем, ако сме банка или финтех",
  topicQuantumComputing:
    "Квантовите изчисления и тяхното въздействие върху цифровата сигурност във финансовата индустрия",
  topicAIBanking: "“Без болка, Без печалба” - Примери от реалния свят за AI и RPA във финансите",
  topicMachineLearningAML: "Използване на машинно обучение и изкуствен интелект за AML контроли",
  topicConvenientPayments: "Теза: Защитава удобните плащания",
  topicSecurePayments: "Теза: Защитава сигурните плащания",
  moderator: "Модератор",
  moderators: "Модератори",
  poweredByBorica: "Подкрепенa от БОРИКА",
  poweredByIbs: "От IBS България",
  poweredByIbs2: "Подкрепенa от IBS България",
  poweredByDais: "Подкрепенa от DAIS Software",
  topicBuildingCorporateCulture:
    "Изграждането на корпоративна култура - важен критерий за справяне с финансовите измами",
  topicCroatiaEuro: "Навигиране при смяна на еврото: вътрешни стратегии и резултати от Хърватия",
  topicMoneyLaundering: "Как да разпознаваме 'прането на пари' в реалния живот: ключови сигнали и примери.",
  // phd,

  // Names
  ericDucoulombier: "Ерик Дюкуломбие",
  dimitarRadev: "Димитър Радев",
  merdihanIsmailov: "Мердихан Исмаилов",
  stoilkaArsova: "Стоилка Арсова",
  hristiyanPrashkov: "Християн Прашков",
  tatyanaIvanova: "Татяна Иванова",
  svetlinLazarov: "Светлин Лазаров",
  radoslavDimitrov: "Радослав Димитров",
  goranAngelov: "Горан Ангелов",
  pavelGeorgiev: "Павел Георгиев",
  borisGoncharov: "Борис Гончаров",
  ivanVulev: "Иван Вулев",
  anastasiiaZhdanova: "Анастасия Жданова",
  alexanderKrastev: "Александър Кръстев",
  ivanaRosancic: "Ивана Росанчич",
  deyanRadev: "Деян Радев",
  chrystallaKazara: "Кристала Казара",
  anestisDimopoulos: "Анестис Димопулос",
  stoyanLozanov: "Стоян Лозанов",
  miglenEvlogiev: "Миглен Евлогиев",
  marinaShideroff: "Марина Шидероф",
  tsvetomirDoskov: "Цветомир Досков",
  pavelKaminsky: "Павел Камински",
  valeryVulchev: "Валери Вълчев",
};
